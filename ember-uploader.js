...
define("ember-uploader/uploader",
  ...
  function(__exports__) {
    ...
    __exports__["default"] = Ember.Object.extend(Ember.Evented, {
      ...
      /**
       * Start upload of files and extra data SERIALLY
       *
       * @param  {object|array} files  One file object or one array of files object
       * @param  {array} extra
       * @return {object}       jquery promise from ajax object
       */
      uploadSerial: function(files, extra) {
        extra = extra || {};
        var url  = get(this, 'url');
        var type = get(this, 'type');
        var headers = get(this, 'headers');
        ...
        for (i = 0; i < files.length; i++) {
          var data = this.setupFormData(files[i], extra);
          promise = this.ajax(url, data, type, headers);
        }
        return promise;
      },
